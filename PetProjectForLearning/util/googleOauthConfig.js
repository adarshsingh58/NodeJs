...
*     Configuration Object ({ clientID, clientSecret, callbackURL }): This first argument provides the essential configuration details needed for the strategy to communicate with Google:
*               clientID: process.env.GOOGLE_OAUTH_CLIENT_ID: This is your application's unique public identifier, obtained from the Google Cloud Console when you set up OAuth credentials. It tells Google which application is requesting authentication. It's being loaded securely from an environment variable.
*               clientSecret: process.env.GOOGLE_OAUTH_CLIENT_SECRET: This is a secret key known only to your application and Google, also obtained from the Google Cloud Console. It's used to verify your application's identity when exchanging codes for tokens. It's also loaded securely from an environment variable.
...
*     Verify Callback Function ((accessToken, refreshToken, profile, done) => { ... }): This second argument is the most important part for your application logic.
...
*                   Exchanging that code (along with your clientID and clientSecret) with Google for an accessToken and potentially a refreshToken.
*                   Using the accessToken to request the user's profile information from Google.
*               Then, it calls this function, passing in the results:
*               accessToken: A short-lived token granted by Google. Your application could use this token to make authorized API calls to Google APIs on behalf of the user (e.g., read calendar events, access contacts, depending on the requested scope). Often, for simple login, you don't need to store or use this token directly yourself.
*               refreshToken: A longer-lived token (if requested and granted by Google - depends on access_type=offline and user consent) that can be used to obtain new accessTokens without requiring the user to log in again. This is crucial for background access but needs to be stored very securely. Note: The code snippet provided incorrectly logs accessToken twice; the second log should likely be refreshToken.
...
module.exports = function (passport) {
    passport.use(new GoogleStrategy({
            ...
            console.log(" Google Oauth Access Token:", accessToken ? "[REDACTED]" : "N/A"); // Avoid logging sensitive tokens directly
            console.log(" Google Oauth Refresh Token:", refreshToken ? "[REDACTED]" : "N/A");
